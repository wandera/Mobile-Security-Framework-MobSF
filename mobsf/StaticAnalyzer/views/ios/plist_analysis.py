...
from plistlib import dumps, load, loads
from re import sub

from biplist import InvalidPlistException, readPlist, writePlistToString
from openstep_parser import OpenStepDecoder

from mobsf.MobSF.utils import find_key_in_dict, is_file_exists
from mobsf.StaticAnalyzer.views.common.shared_func import is_secret
...
def get_bundle_id(pobj, src):
    ...
    if not any(tmpl in bundle_id_og for tmpl in skip_chars) and len(bundle_id_og) > 1:
...
def plist_analysis(src, is_source):
    ...
    try:
        ...
        plist_info["bin_name"] = plist_obj.get(
            "CFBundleDisplayName", ""
        ...
        for plist_file_ in plist_files:
            ...
            ats_inseccon, ats_transport_security_info = check_transport_security(
                plist_obj_
            )
            ats += ats_inseccon
...
def get_plist_secrets(xml_string):
    ...
    for index, line in enumerate(xml_list):
        if "<key>" in line and is_secret(_remove_tags(line)):
            nxt = index + 1
            value = _remove_tags(xml_list[nxt]) if nxt < len(xml_list) else False
            if value and " " not in value:
                result_list.append(f"{_remove_tags(line)} : {_remove_tags(value)}")
    return result_list
