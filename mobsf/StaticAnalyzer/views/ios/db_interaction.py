...
from mobsf.StaticAnalyzer.models import RecentScansDB, StaticAnalyzerIOS
...
def get_context_from_db_entry(db_entry):
    ...
    try:
        ...
        code = process_suppression(python_dict(db_entry[0].CODE_ANALYSIS), bundle_id)
        binary = process_suppression(
            python_dict(db_entry[0].BINARY_ANALYSIS), bundle_id
        )
        context = {
            ...
            "bundle_supported_platforms": python_list(
                db_entry[0].BUNDLE_SUPPORTED_PLATFORMS
            ),
            ...
            "transport_security_info": python_list(db_entry[0].TRANSPORT_SECURITY_INFO),
...
def get_context_from_analysis(app_dict, info_dict, code_dict, bin_dict, all_files):
    """Get the context for IPA/ZIP from analysis results."""
    try:
        bundle_id = info_dict["id"]
        code = process_suppression(code_dict["code_anal"], bundle_id)
        binary = process_suppression(bin_dict["bin_code_analysis"], bundle_id)
        context = {
            "version": settings.MOBSF_VER,
            "title": "Static Analysis",
            ...
            "binary_analysis": binary,
            "transport_security_info": info_dict["transport_security_info"],
...
def save_or_update(update_type, app_dict, info_dict, code_dict, bin_dict, all_files):
    ...
    try:
        ...
        if update_type == "save":
            db_entry = StaticAnalyzerIOS.objects.filter(MD5=app_dict["md5_hash"])
